...
LedDevice * LedDeviceFactory::construct(const Json::Value & deviceConfig)
{
	...
	else if (type == "philipshue")
	{
		...
		const float brightnessFactor = deviceConfig.get("brightnessFactor", 1.0f).asFloat();
		...
		device = new LedDevicePhilipsHue(output, username, switchOffOnBlack, brightnessFactor, transitiontime, lightIds);
	}
